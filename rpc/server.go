...
import (
	...
	rpcapi "github.com/smartbch/ganychain/rpc/api"
)
...
func NewServer(rpcAddr, wsAddr, rpcAddrSecure, wsAddrSecure, corsDomain, certFile, keyFile string,
	serverCfg *tmrpcserver.Config, backend backend.BackendService,
	logger tmlog.Logger, httpAPI string, wsAPI string) tmservice.Service {
...
func (server *Server) startHTTPAndHTTPS(apis []gethrpc.API) (err error) {
	...
	server.httpListener, err = tmrpcserver.Listen(
		server.rpcAddr, server.serverConfig.MaxOpenConnections)
	if err != nil {
		return err
	}
	go func() {
	...
	if server.rpcHttpsAddr != "off" {
		server.httpsListener, err = tmrpcserver.Listen(
			server.rpcHttpsAddr, server.serverConfig.MaxOpenConnections)
		if err != nil {
			return err
		}
		go func() {
...
func (server *Server) startWSAndWSS(apis []gethrpc.API) (err error) {
	...
	server.wsListener, err = tmrpcserver.Listen(
		server.wsAddr, server.serverConfig.MaxOpenConnections)
	if err != nil {
		return err
	}
	go func() {
	...
	if server.wssAddr != "off" {
		server.wssListener, err = tmrpcserver.Listen(
			server.wssAddr, server.serverConfig.MaxOpenConnections)
		if err != nil {
			return err
		}
		go func() {
